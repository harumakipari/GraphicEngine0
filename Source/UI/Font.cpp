...
Font::Font(ID3D11Device* device, const char* filename, int maxSpriteCount)
{
    // 頂点シェーダー
    {
        // ファイルを開く
        FILE* fp = nullptr;
        fopen_s(&fp, "./Data/Shaders/FontVS.cso", "rb");
    ...
    // ピクセルシェーダー
    {
        // ファイルを開く
        FILE* fp = nullptr;
        fopen_s(&fp, "./Data/Shaders/FontPS.cso", "rb");
        _ASSERT_EXPR_A(fp, "CSO File not found");
...
#else
    float viewX, viewY;
    Graphics::GetViewport(viewX, viewY, screenWidth, screenHeight);
#endif // 0
...
void Font::Draw(float x, float y, const wchar_t* string, DirectX::XMFLOAT4 color, float scale, float pivotX, float pivotY)
{
    ...
    float scale_ui = std::min<float>(screenWidth / DESIGN_W, screenHeight / DESIGN_H);
    ...
    for (size_t i = 0; i < length; ++i)
    {
        ...
        currentVertex[1].position.x = positionX + w;
        ...
        currentVertex[2].position.y = positionY + h;
        ...
        currentVertex[3].position.x = positionX + w;
        currentVertex[3].position.y = positionY + h;
