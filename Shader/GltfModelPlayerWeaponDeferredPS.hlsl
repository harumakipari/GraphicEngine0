...
GBUFFER_PS_OUT main(VS_OUT pin, bool isFrontFace : SV_IsFrontFace)
{
    ...
    // 全体をほんの少し
    emissive += cpuColor.rgb * 0.2;

    // 剣先ほど強い
    float tip = smoothstep(0.3, 1.0, pin.texcoord.y);
    emissive += cpuColor.rgb * emissionPower * tip;

    // リムライト
    float3 V = normalize(cameraPosition - pin.wPosition.xyz);
    float rim = pow(1.0 - saturate(dot(N, V)), 4.0);
    emissive += cpuColor.rgb * rim * 0.4;

    pout.emissive = float4(emissive, GBUFFER_FLAG_NORMAL);
